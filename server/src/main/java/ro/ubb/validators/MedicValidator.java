...
public class MedicValidator implements Validator<Medic> {
    ...
    */
    private String emailRegex = "^[a-zA-Z0-9]+"+"(?:[a-zA-Z0-9._]+)@" +
            "(?:[a-zA-Z0-9-]{1,}\\.)"+"\\w{2,7}$";
    ...
    @Override
    public void validate(Medic user) throws ValidatorException {
...
//        Pattern nameMatcher = Pattern.compile(nameRegEx);
        if (!user.getName().matches(nameRegEx) && user.getName().length() > 25)
    ...
    @Override
    public Medic checkEmptyBeforeUpdate(Medic updateUser, Medic beforeUpdate){
        String name = Optional.ofNullable(updateUser.getName()).get();
        String userName = Optional.ofNullable(updateUser.getUserName()).get();
        String password = Optional.ofNullable(updateUser.getPassword()).get();
        String email = Optional.ofNullable(updateUser.getEmail()).get();
        String specialty = String.valueOf(Optional.ofNullable(updateUser.getSpecialty()).get());
        String beginningDate = String.valueOf(Optional.ofNullable(updateUser.getBeginningDate()).get());
        String dateOfBirth = String.valueOf(Optional.ofNullable(updateUser.getDateOfBirth()).get());;
        String isActive = String.valueOf(Optional.ofNullable(updateUser.getIsActive()).get());;
